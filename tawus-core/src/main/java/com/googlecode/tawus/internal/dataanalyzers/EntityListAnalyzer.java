...
/**
 * Entity list analyzer
 * This analyzer is inserted into DataTypeAnalyzer Service
...
public class EntityListAnalyzer implements DataTypeAnalyzer {
   ...
   /**
    * {@inheritDoc}
    * This selects any collection type which has parameter of type entity
    */
   public String identifyDataType(PropertyAdapter propertyAdapter){
      Class<?> propertyType = propertyAdapter.getType();
      if(Collection.class.isAssignableFrom(propertyType)){
         try {
            Type [] type = ((ParameterizedType)propertyAdapter.getDeclaringClass().
               getDeclaredField(propertyAdapter.getName()).getGenericType()).getActualTypeArguments();

...
public class GenericListValueEncoder<T> implements ValueEncoder<T> {
   ...
   public GenericListValueEncoder(List<T> list, PropertyAccess propertyAccess,
         @SuppressWarnings("rawtypes") Class type, String id) {
      this.propertyAdapter = propertyAccess.getAdapter(type)
            .getPropertyAdapter(id);
      this.list = list;
   }
